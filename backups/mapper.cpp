/*
 * Apple Keyboard Key Mapper — v18
 *
 * Interactive key mapping tool for the M0116.
 * Press a key on the Apple keyboard → type its name on your Mac.
 * Auto-detects Enter, Space, Tab from raw keystrokes.
 *
 * Hardware: 8x10 matrix + 4 direct modifier pins (same as v17)
 */
...
const char* DIRECT_PINS[]   = {"pin6/GPIO33", "pin23/GPIO21", "pin37/GPIO25", "pin30/GPIO14"};
...
// Read current state into debounce buffers without triggering events.
// Prevents re-detection of held keys after returning to WAITING_FOR_KEY.
void silentScan() {
...
// Check one direct pin; returns true on new press
bool checkDirect(int pin, bool &raw, bool &st, unsigned long &db) {
  ...
  if ((millis() - db) >= DEBOUNCE_MS && raw != st) {
    st = raw;
    return st; // only fire on press, not release
  }
  return false;
}
...
void exportMap() {
  ...
  Serial.println("const char* MATRIX_KEYS[8][10] = {");
  for (int r = 0; r < NUM_ROWS; r++) {
    Serial.print("  /* row %d */ {");
...
void printHelp() {
  Serial.println();
  Serial.println("==========================================");
  Serial.println("Apple Keyboard Key Mapper — v18");
  Serial.println("==========================================");
  Serial.println();
...
void handleSerial() {
  ...
  // Keep raw input for special key detection
  ...
  // Commands while waiting for Apple keyboard key press
  input.trim();
